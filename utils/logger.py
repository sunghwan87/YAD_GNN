...
class LoggerSTAGIN(object):
    ...
    def get(self, k=None, initialize=False):
        if self.k_fold is None:
            ...
            subj = np.concatenate(self.samples['subj'])
        else:
            if k is None:
                true, pred, prob, subj = {}, {}, {}, {}
                for k in range(self.k_fold):
                    ...
                    subj[k] = np.concatenate(self.samples[k]['subj'])
            else:
                true = np.concatenate(self.samples[k]['true'])
                pred = np.concatenate(self.samples[k]['pred'])
                prob = np.concatenate(self.samples[k]['prob'])
                subj = np.concatenate(self.samples[k]['subj'])

        if initialize:
            self.initialize(k)

        return dict(true=true, pred=pred, prob=prob, subj=subj)
