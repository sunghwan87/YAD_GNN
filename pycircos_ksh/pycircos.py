...
from pycircos_ksh.utils import patch_to_pts
...
class Garc:
    ...
    def __init__(self, arc_id=None, record=None, size=1000, interspace=3, raxis_range=(500, 550), facecolor=None, edgecolor="#303030", linewidth=0.75, label=None, labelposition=0, labelsize=10, label_visible=False, labelrotation=True):
        """
...
class Gcircle:
        ...
        for i, key in enumerate(self._garc_dict.keys()):
            ...
            if self._garc_dict[key].label_visible == True:
                ...
                if 180 < rot < 360:
                    ...
                    rot = -1 * rot+90
                    ha = "left"
                if not self._garc_dict[key].labelrotation:
                    rot=0
                    ha="center"
                height = bottom + height/2 + self._garc_dict[key].labelposition
                self.ax.text(pos + width/2, height, self._garc_dict[key].label, rotation=rot, rotation_mode="anchor", ha=ha, va="center", fontsize=self._garc_dict[key].labelsize)
    ...
    def chord_plot(self, start_list, end_list, facecolor=None, edgecolor=None, linewidth=0.0, directed=True, narrow=3):
        """
        ...
        if sstart == ostart: 
            ...
            if directed:
                patch = mpatches.FancyArrowPatch(path=path, arrowstyle='->', facecolor=facecolor, linewidth=linewidth, edgecolor=edgecolor, zorder=0)
                intrp_polar = patch_to_pts(patch)
